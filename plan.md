...
| D3 生物特征异常 | **已完成**（口腔预留） | 7 tests | `src/biological_anomaly/` |
...
### 3. 生物特征细节异常 — **已完成**（口腔模块预留）

- **实现方案：**
  - **眼睛异常：** EAR（Eye Aspect Ratio）检测长期不眨眼（>90帧/3秒）
  - **手指畸变：** 手指数量 ≠ 5 检测、关节角度超出人体工学范围 (0°~180°)
  - **口腔异常：** 接口已预留，待实现
  - 评分：异常帧占比反向评分 `score = 1.0 - anomaly_count / n_frames`

**下一步：**
- [ ] 实现口腔异常检测：牙齿区域边缘检测 + 连通域分析
- [ ] 眼睛增强：双眼不同步检测、瞳孔固定/注视方向不一致
- [ ] 手指增强：骨骼长度比突变检测（容忍度 15%）、自穿透检测
- [ ] 采集实际 AIGC 视频样本进行有效性验证
