...
| D5 物理常识           | **已完成**（VLM+CoT）      | 9 tests  | `src/physics_consistency/`    |
| D6 环境一致性         | **已完成**                 | 4 tests  | `src/background_consistency/` |
| D7 时间一致性         | **已完成**（TCS-lite）     | 2 tests  | `src/temporal_coherence/`     |
...
### 5. 物理常识与动力学一致性 — **已完成**（VLM+CoT 重构）

- **实现方案（VLM 为主 + 漂移辅助）：**
  - **VLM 思维链判定（核心）：** 四步 CoT 推理 — 场景识别 → 运动方向分析 → 物理规律检查 → 综合判定
    - 运动方向异常：车辆逆行、不合理同步运动、行人方向异常
    - 物理违规：刚体穿模、悬浮、光影矛盾、流体异常、物质守恒
    - 输出细粒度 0-1 评分 + 异常列表（含类型、严重度、置信度）
  - **像素漂移检测（辅助信号）：** 静态区域光流分析，检测到漂移时注入 VLM prompt 作为上下文提示
  - **降级策略：** VLM 不可用时回退为漂移检测评分
  - ~~重力方向一致性：已移除（tracking extractor 依赖不稳定，VLM 已覆盖此场景）~~

**下一步：**

- 收集实际 AIGC 视频样本验证 CoT prompt 的判定准确率
- 优化 prompt：针对高频误报/漏报场景迭代
- 考虑引入多轮对话或 self-consistency 提升判定鲁棒性
...
### 7. 时间一致性（目标异常出现/消失） — **已完成**（TCS-lite）

- **实现方案：**
  - **Grounding DINO 检测：** 在抽样帧中检测所有物体（通用目标检测，不限于人脸）
  - **IoU 追踪：** 按 IoU 相似度将检测框串成轨迹，跨帧关联
  - **事件分类：** 区分正常 vs 异常出现/消失
    - `edge_emerge/edge_vanish` — 从画面边缘进出（正常，如行人走出画面）
    - `small_emerge/small_vanish` — 物体逐渐放大/缩小（正常，如物体靠近/远离）
    - `detect_gap` — 检测间隔导致的断点（正常，采样率限制）
    - **`abnormal`** — 物体突然凭空出现/消失（异常 ⚠️，如月亮突然消失、鸟儿凭空出现）
  - **评分：** `score = 1.0 - (异常事件数 / 总事件数)`，异常事件越多评分越低

**应用场景：**

- 检测 AI 生成视频中的物质守恒违反（物体凭空出现/消失）
- 补充 D5 物理常识的语义判定（低级视觉信号）
- 适用于任何包含运动物体的视频（人、动物、车辆、天体等）

**下一步：**

- 优化 Grounding DINO 的检测阈值和 NMS 参数
- 支持自定义目标类别过滤（如仅检测人、车等）
- 增强小物体检测鲁棒性（如鸟、月亮等）
- 与 D5 物理常识模块联动：异常事件作为 VLM 上下文提示

---

## 统一流水线 — **已完成**

- `EvaluationPipeline.evaluate(video_path)` 一键七维度评测
- 不适用维度自动跳过 + 权重归一化重分配
